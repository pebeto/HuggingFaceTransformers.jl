#!/usr/bin/env python3
"""Record reference logits for HuggingFaceTransformers.jl's Mixtral parity tests.

Usage:
    python3 test/fixtures/record_mixtral_parity.py [VARIANT]

VARIANT is one of: 8x7b, 8x22b (default: 8x7b).
...
Resource cost (fp32 weights + activations):
    8x7b:  ~90 GB download, ~95 GB RAM   (workstation only)
    8x22b: ~280 GB download, ~290 GB RAM (server only)

Mixtral checkpoints are large because every expert is materialized in
the file even though only `top_k` of them run per token at inference
...
VARIANTS = {
    "8x7b":  ("mistralai/Mixtral-8x7B-Instruct-v0.1",
              "mixtral_8x7b_instruct_v01_parity.json"),
    "8x22b": ("mistralai/Mixtral-8x22B-Instruct-v0.1",
              "mixtral_8x22b_instruct_v01_parity.json"),
}
...
def main(variant):
    ...
    print("(This is a large MoE checkpoint — expect a multi-minute load.)")
...
if __name__ == "__main__":
    variant = sys.argv[1] if len(sys.argv) > 1 else "8x7b"
    main(variant)
