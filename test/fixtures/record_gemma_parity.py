#!/usr/bin/env python3
"""Record reference logits for HuggingFaceTransformers.jl's Gemma2 parity tests.

Usage:
    python3 test/fixtures/record_gemma_parity.py [VARIANT]

VARIANT is one of: 2B, 9B, 27B (default: 2B).

Requirements:
    pip install transformers torch
    huggingface-cli login         # or set HF_TOKEN — Gemma weights are
                                  # gated; accept Google's terms on the
                                  # model page before downloading.

Resource cost (fp32 weights + activations):
    2B:  ~10 GB download,  ~12 GB RAM
    9B:  ~37 GB download,  ~40 GB RAM   (server-class)
    27B: ~108 GB download, ~120 GB RAM  (workstation/server only)
...
VARIANTS = {
    "2B":  ("google/gemma-2-2b-it",  "gemma_2_2b_it_parity.json"),
    "9B":  ("google/gemma-2-9b-it",  "gemma_2_9b_it_parity.json"),
    "27B": ("google/gemma-2-27b-it", "gemma_2_27b_it_parity.json"),
}
...
if __name__ == "__main__":
    variant = sys.argv[1] if len(sys.argv) > 1 else "2B"
    main(variant)
