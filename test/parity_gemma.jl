...
const VARIANTS = (
    ("2B", "gemma_2_2b_it_parity.json"),
    ("9B", "gemma_2_9b_it_parity.json"),
    ("27B", "gemma_2_27b_it_parity.json"),
)
...
const SELECTED = _selected_variants(get(ENV, "ALLSPARK_TEST_PARITY_GEMMA", ""))

function _load_gemma_config(snapshot_dir::AbstractString)
...
function _run_variant(name::AbstractString, fixture_filename::AbstractString)
    ...
    @testset verbose = true "Gemma2-$(name) ($(repo_id))" begin
        ...
        @testset "last-position logits within $(tolerance)" begin
            cfg = _load_gemma_config(snapshot_dir)
...
isempty(unknown) || error(
    "Unknown ALLSPARK_TEST_PARITY_GEMMA variant(s): $(unknown). " *
