...
const VARIANTS = (
    ("mini-4k", "phi_3_mini_4k_instruct_parity.json"),
    ("medium-4k", "phi_3_medium_4k_instruct_parity.json"),
)
...
function _load_phi3_config(snapshot_dir::AbstractString)
    ...
    # Bail loudly on partial-rotary or longrope variants — both need their
    # own implementation.
...
function _run_variant(name::AbstractString, fixture_filename::AbstractString)
    ...
    @testset verbose = true "Phi-3-$(name) ($(repo_id))" begin
        ...
        @testset "last-position logits within $(tolerance)" begin
            cfg = _load_phi3_config(snapshot_dir)
