...
# v0.3 disables it — both exercise the same code path with different config.
const VARIANTS = (
    ("v0.2", "mistral_7b_instruct_v02_parity.json"),
    ("v0.3", "mistral_7b_instruct_v03_parity.json"),
)
...
function _run_variant(name::AbstractString, fixture_filename::AbstractString)
    ...
    @testset verbose = true "Mistral-7B-Instruct-$(name) ($(repo_id))" begin
        ...
        @testset "last-position logits within $(tolerance)" begin
            cfg = _load_mistral_config(snapshot_dir)
