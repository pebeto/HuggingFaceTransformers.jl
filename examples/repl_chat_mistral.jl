...
#   julia --project=. examples/repl_chat_mistral.jl mistralai/Mistral-7B-Instruct-v0.2
#
# Authentication:
#   export HF_TOKEN=hf_...      # or `huggingface-cli login`
#
# Resource cost (fp32):
#   Mistral-7B-Instruct-v0.2:  ~30 GB download, ~30 GB peak RAM.
...
# Fallback template for when the bundled chat_template trips on Jinja
# features we don't yet support. Mirrors Mistral's [INST] / [/INST] form.
const FALLBACK_MISTRAL_TEMPLATE = raw"""
{{- bos_token -}}
...
function load_mistral_config(snapshot_dir::AbstractString)
...
function load_chat_template(snapshot_dir::AbstractString)
    ...
    return ChatTemplate(FALLBACK_MISTRAL_TEMPLATE), bos, eos
end
...
function main(repo_id::AbstractString=DEFAULT_MODEL)
    ...
    cfg = load_mistral_config(snapshot_dir)
