...
#   julia --project=. examples/repl_chat_phi3.jl microsoft/Phi-3-medium-4k-instruct
#
# Authentication: Phi-3 weights are open; no HF token needed.
#
# Resource cost (fp32):
#   Phi-3-mini-4k-instruct   (3.8B): ~16 GB download, ~18 GB RAM
...
# Fallback ChatML-like template Phi-3 actually uses. Markers are
# <|user|>, <|assistant|>, <|system|>, terminator is <|end|>.
const FALLBACK_PHI3_TEMPLATE = raw"""
...
function load_chat_template(snapshot_dir::AbstractString)
    ...
    return ChatTemplate(FALLBACK_PHI3_TEMPLATE)
end
